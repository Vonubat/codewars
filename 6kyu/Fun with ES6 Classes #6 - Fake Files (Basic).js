...
class File {
  ...
  get filename() {
    const fileName = this._fullName.slice(0, this._fullName.lastIndexOf('.'));
    return fileName;
  }

  get extension() {
    const extension = this._fullName.slice(this._fullName.lastIndexOf('.') + 1);
    return extension;
  }
  ...
  gets() {
    ...
    return lines[this._counterLines++];
  }

  getc() {
    const characters = this._contents.split('');
    return characters[this._counterChar++];
  }
}
